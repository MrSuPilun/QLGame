<div id="layoutAuthentication">
  <div id="layoutAuthentication_content">
    <main>
      <div class="container">
        <div class="row justify-content-center">
          <div class="col-lg-5">
            <div class="card shadow-lg border-0 rounded-lg mt-5 bg-dark">
              <div class="card-header">
                <div class="text-center font-weight-light my-4">
                  <a class="navbar-brand" href="web.php"><img src="data:image/png;base64," class="d-inline-block align-top" alt="" width="42" height="50"></a> <br>
                  <h5 class="mt-3" style="color:white;">Sign in with your account</h5>
                </div>
              </div>
              <div class="card-body">
                <form align='center' method='POST'>
                  <div class="form-floating mb-3">

                    <div class="form-outline mb-4">
                      <input type="text" id="tenDN" name="tenDN" class="form-control-lg btn-dark" placeholder="Sign In name" />
                    </div>

                    <div class="form-outline mb-4">
                      <input type="password" id="matKhau" name="matKhau" class="form-control-lg btn-dark" placeholder="Password" />
                    </div>

                    <div class="mt-4 mb-0">
                      <div class="d-grid"><input type="submit" id="dangNhap" name="dangNhap" value="Sign in now" class="btn-primary form-control-lg" /></div>
                    </div>

                  </div>
                </form>
              </div>
              <div class="card-footer text-center py-3">
                <form method="POST">
                  <div class="small" style="color:white;">Don't have an account? <button name="register" style="color:white;font-weight:bold;background-color: transparent;">Sign Up</button></div>
                </form>
              </div>
            </div>
          </div>
        </div>
      </div>
    </main>
  </div>
</div>